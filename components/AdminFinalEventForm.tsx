...
export default function AdminFinalEventForm({
  ...
  const startMap = useMemo(() => {
    ...
    for (const r of startScores ?? []) m.set(r.season_player_id, Number(r.start_score ?? 0));
    return m;
  }, [startScores]);
  ...
  async function callApi(mode: "save" | "lock" | "unlock") {
    ...
    try {
      ...
      if (mode === "unlock") setMsg("Finalen är upplåst ✅");
      else if (mode === "lock") setMsg("Finalen är låst ✅");
      else setMsg("Sparat ✅");
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white/70 text-left">
        <strong>Final</strong>: Mata in <strong>bruttoslag</strong>.{" "}
        <strong>Adjusted = Brutto − HCP + Start</strong>.{" "}
        <strong>Särspel placering</strong> används om adjusted blir lika.
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-white/10 bg-white/5">
        {players.map((p) => {
          ...
          const start = startMap.get(p.season_player_id) ?? 0;

          return (
            <div key={p.season_player_id} className="border-b border-white/10 px-4 py-4 last:border-b-0">
              <div className="mb-2">
                <div className="font-semibold">{p.name}</div>
                <div className="text-sm text-white/60">HCP {p.hcp.toFixed(1)} • Start {start}</div>
              </div>
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="w-24 rounded-lg border border-white/10 bg-black/30 px-2 py-1 text-sm tabular-nums focus:border-white/30 focus:outline-none"
                    />
                  </div>
